...
export default function App() {
  ...
  const [answers, setAnswers] = useState<Answer[]>([]); // pour Révision

  const max = corpusSize(content);

  // Timer (examen uniquement, basé sur la longueur réelle de la session)
  ...
  function start() {
    ...
    if (max < 4) return; // il faut au moins 4 items pour 4 choix
  ...
  function startFromQuestions(qs: Q[]) {
    ...
    setSession(qs);
    setI(0); setScore(0); setAnswers([]);
    setMode("entrainement"); // révision en mode entraînement
    setScreen("quiz");
  }
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow p-6 space-y-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                disabled={notEnough}
                className={
                  "px-5 py-2 rounded-lg text-white text-sm font-medium transition " +
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center p-6 pb-16">
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow p-6 space-y-6 w-full max-w-xl text-center text-slate-900 dark:text-slate-100">
          ...
          <div className="flex items-center justify-center gap-3">
            <button onClick={() => setScreen("home")} className="px-4 py-2 rounded-lg bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-100 hover:bg-slate-300 dark:hover:bg-slate-600">
              Accueil
            </button>
            <button onClick={() => setScreen("review")} className="px-4 py-2 rounded-lg bg-amber-600 text-white hover:bg-amber-500">
              Révision ({wrongs.length})
            </button>
            <button onClick={() => start()} className="px-4 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-500">
              Rejouer
            </button>
          </div>
        </div>
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center p-6 pb-16">
        <div className="w-full max-w-3xl space-y-4 text-slate-900 dark:text-slate-100">
          ...
          {wrongs.length === 0 ? (
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow p-6 text-center text-slate-600 dark:text-slate-300">
                    ...
                    <div key={idx} className="rounded-xl border border-slate-200 dark:border-slate-700 p-4">
                      ...
                      <ul className="grid gap-2">
                        {a.q.choices.map((c, j) => {
                          ...
                          const cls = isCorrect
                            ? "border-emerald-300 dark:border-emerald-400/40 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-800 dark:text-emerald-300"
                            : isChosen
                            ? "border-rose-300 dark:border-rose-400/40 bg-rose-50 dark:bg-rose-950/30 text-rose-700 dark:text-rose-300"
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 flex items-center justify-center p-6 pb-16">
      <div className="w-full max-w-2xl space-y-4">
        <header className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button onClick={() => setScreen("home")} className="text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200">← Accueil</button>
            <h1 className="text-2xl font-semibold text-indigo-600">English Prep</h1>
          ...
          <div className="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-300">
            {mode === "examen" && (
              <span
                className={"font-mono tabular-nums px-2 py-1 rounded " + (low ? "bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-200" : "bg-slate-100 dark:bg-slate-700")}
