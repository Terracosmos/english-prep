...
type SessionMode = "entrainement" | "examen" | "examen_officiel";
...
const isExam = (m: SessionMode) => m !== "entrainement";

// fabriquer 1 question par item, distracteurs pris dans la même catégorie
...
function corpusSize(content: ContentMode) {
  if (content === "mots") return vocab.length;
  if (content === "expressions") return expressions.length;
  return definitions.length;
}
function corpusSizeTotalEligible() {
  // on ignore les catégories < 4 (pas assez pour 3 distracteurs)
  const v = vocab.length >= 4 ? vocab.length : 0;
  const e = expressions.length >= 4 ? expressions.length : 0;
  const d = definitions.length >= 4 ? definitions.length : 0;
  return v + e + d;
}
...
function createMixedExam(size: number): Q[] {
  // génère toutes les questions possibles dans chaque catégorie éligible, puis en prend N au hasard (mélange global)
  const qsV = vocab.length >= 4 ? vocab.map((it) => makeQuestionFromVocab(it, vocab)) : [];
  const qsE = expressions.length >= 4 ? expressions.map((it) => makeQuestionFromExpr(it, expressions)) : [];
  const qsD = definitions.length >= 4 ? definitions.map((it) => makeQuestionFromDef(it, definitions)) : [];
  const all = [...qsV, ...qsE, ...qsD];
  return pickN(all, Math.min(size, all.length));
}
...
export default function App() {
  ...
  // max disponible selon le mode
  const max = mode === "examen_officiel" ? corpusSizeTotalEligible() : corpusSize(content);

  // timer (examen & examen_officiel)
  const totalSecondsQuiz = useMemo(() => {
    if (screen !== "quiz" || !isExam(mode)) return 0;
  ...
  useEffect(() => {
    if (screen !== "quiz" || !isExam(mode)) return;
  ...
  function start() {
    ...
    if (max < 4) return;
    if (mode === "examen_officiel") {
      setSession(createMixedExam(size));
    } else {
  ...
  // ---------- HOME ----------
  if (screen === "home") {
    const estimated = isExam(mode) ? Math.min(count, max) * SECONDS_PER_QUESTION : 0;
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center p-6 pb-16">
        ...
        <div className="w-full max-w-2xl space-y-6 text-slate-900 dark:text-slate-100">
          <header className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm text-slate-500 dark:text-slate-400">
                {mode === "examen_officiel" ? "Corpus total" : `Corpus ${content}`} : {max}
              </span>
              <ThemeToggle />
            </div>
          </header>
          ...
          <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur rounded-2xl ring-1 ring-slate-200/70 dark:ring-white/10 shadow p-6 space-y-6">
            ...
            {/* Contenu (désactivé si examen_officiel) */}
            <div className={"space-y-2 " + (mode === "examen_officiel" ? "opacity-50 pointer-events-none" : "")}>
              ...
              <div className="inline-flex rounded-xl bg-slate-100 dark:bg-slate-700 p-1">
                {(["mots","expressions","definitions"] as ContentMode[]).map((c) => (
                  <button
                    ...
                    aria-disabled={mode === "examen_officiel"}
                  >
                    {c}
                  </button>
                ))}
              ...
              <div className="inline-flex rounded-xl bg-slate-100 dark:bg-slate-700 p-1">
                {modeOptions.map(({ value, label }) => (
                  <button
                    ...
                    className={
                      "px-3 py-1.5 rounded-lg text-sm font-medium capitalize transition " +
                      (mode === value ? "bg-white dark:bg-slate-900 shadow text-slate-900 dark:text-slate-100"
                                      : "text-slate-600 dark:text-slate-200 hover:text-slate-800 dark:hover:text-white")
                    }
                    title={value === "examen_officiel" ? "Mélange des 3 catégories" : undefined}
                  >
                    {label}
                  </button>
                ))}
                ...
                {COUNTS.map((n) => {
                  ...
                  return (
                    <button
                      ...
                      title={disabled ? `Seulement ${max} dispo` : `Utiliser ${n} questions`}
                    >
                      {n}
                    </button>
                  );
            ...
            {notEnough && (
              <div className="text-xs text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded p-2">
                Pas assez d’éléments pour générer des questions.
              </div>
            )}
          </div>
        </div>
  ...
  const low = isExam(mode) && totalSecondsQuiz > 0 && timeLeft <= Math.max(10, Math.floor(totalSecondsQuiz * 0.1));
  const badgeText = mode === "examen_officiel" ? "mixte · examen" : `${content} · ${mode}`;
      ...
      <div className="w-full max-w-2xl space-y-4">
        <header className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-200 capitalize">
              {badgeText}
            </span>
          </div>
          <div className="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-300">
            {isExam(mode) && (
              <span
                className={
                  "font-mono tabular-nums px-2 py-1 rounded ring-1 ring-slate-200/70 dark:ring-white/10 " +
                  (low ? "bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-200" : "bg-white/70 dark:bg-slate-800/70")
                }
