...
// Reprend les props du motion.button mais force children => ReactNode
type MotionBtnProps = ComponentPropsWithoutRef<typeof motion.button>;
type Props = Omit<MotionBtnProps, "children"> & {
  status?: Status;
  children?: ReactNode;
};
  ...
  return (
    <motion.button
      ...
      className={
        ...
        "ring-1 ring-slate-200/70 dark:ring-white/10 shadow-sm p-4 text-left hover:shadow " +
        "text-slate-900 dark:text-slate-100 flex items-center justify-between " +
        ring + " " + className
      }
      {...props}
    >
      <span className="pr-3">{children as ReactNode}</span>
      {status === "correct" && <CheckCircle2 className="text-emerald-500" size={20} aria-hidden={true} />}
      {status === "wrong" && <XCircle className="text-rose-500" size={20} aria-hidden={true} />}
    </motion.button>
  );
}
